...
export function KpiCards({ fx, cpiYoY, geoRisk, marketTemperature }: { fx: number; cpiYoY: number; geoRisk: number; marketTemperature: number }) {
  const cards: Kpi[] = [
    ...
    { label: "USD/JPY", value: fx.toFixed(1), sub: "最新FX API / FRED / 手入力を分離表示", tone: "info" },
    { label: "米国CPI YoY", value: `${cpiYoY.toFixed(1)}%`, sub: "CPIAUCSLからYoY自動計算", tone: cpiYoY >= 3.5 ? "warn" : "good" },
    { label: "地政学リスク", value: `${Math.round(geoRisk)}`, sub: "GDELT + 公式RSSのリスク語で算出", tone: toneFor(geoRisk) },
    { label: "購買市場温度", value: `${Math.round(marketTemperature)}`, sub: "為替・物価・雇用・基地/ニュースを合成", tone: toneFor(marketTemperature) }
  ];
