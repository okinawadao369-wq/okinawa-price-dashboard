...
export function DailyMemoPanel({ fredData, newsScores, marketTemperature, fxOverride }: { fredData: FredPoint[]; newsScores: TopicScore[]; marketTemperature: number; fxOverride?: number }) {
  ...
  const cacheTopics = newsScores.filter((score) => score.status === "cache").length;
  const fallbackTopics = newsScores.filter((score) => score.status === "fallback").length;
  const dataMode = liveTopics === newsScores.length
    ? "GDELT live"
    : liveTopics > 0
      ? `GDELT partial live ${liveTopics}/${newsScores.length}`
      : `GDELT cache/fallback cache=${cacheTopics} fallback=${fallbackTopics}`;
  const memo = `【自動分析メモ】
データ状態: FRED live=${fredLive}/${fredData.length} cache=${fredCache} fallback=${fredFallback}、${dataMode}。cache/fallbackの場合、該当スコアは前回取得値または代替スコアとして扱う。
